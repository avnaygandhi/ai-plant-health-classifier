...
# Import your FastAPI app object from your backend module
# Adjust this import path if your file structure is different (e.g., from main import app)
from Backend.main import app


# Background thread function to run the FastAPI server
def start_fastapi():
  uvicorn.run(app, host="127.0.0.1", port=8000, log_level="error")


# Launch the FastAPI server once per session state initialization
if "fastapi_started" not in st.session_state:
  api_thread = threading.Thread(target=start_fastapi, daemon=True)
  api_thread.start()
  st.session_state["fastapi_started"] = True
  time.sleep(2)  # Short delay to allow uvicorn to bind to port 8000
...
st.markdown(
    "<div class='sub-header'>Upload a leaf image to instantly identify species"
    " and receive health assessments.</div>",
    unsafe_allow_html=True,
)
...
with st.sidebar:
  ...
  if st.button("🛑 Stop Server & Exit", width="stretch"):
